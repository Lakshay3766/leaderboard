...
""", unsafe_allow_html=True)
search_query = st.text_input("Search for your name:")

# Filter the DataFrame based on the search query
if search_query:
    filtered_df = leaderboard_df[leaderboard_df['Name'].str.contains(search_query, case=False, na=False)]
else:
    filtered_df = leaderboard_df

# Display the leaderboard if data is available
if not filtered_df.empty:
    st.table(filtered_df[['Rank', 'Name', 'League', 'Points']].set_index('Rank'))
else:
